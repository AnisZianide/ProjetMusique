...
void set_status(const char *message, int is_error) {
    ...
    else markup = g_markup_printf_escaped("<span foreground='#1db954'><b>%s</b></span>", message); // Vert Spotify
    gtk_label_set_markup(GTK_LABEL(lbl_status), markup);
    g_free(markup);
}

// --- AUTO-SCROLL (Version safe V19) ---
gboolean on_scroll_timer(gpointer user_data) {
    (void)user_data;
    if (!is_loaded || is_paused) return TRUE;
    // Calcul temporairement désactivé pour éviter l'erreur miniaudio
    return TRUE;
}
...
char* get_chemin_from_row(GtkListBoxRow *r) {
    if(!r)return NULL; GtkWidget *b=gtk_list_box_row_get_child(r); GtkWidget *bt=gtk_widget_get_last_child(b);
...
void on_btn_next_clicked(GtkButton *b, gpointer u) { (void)b; (void)u;
    if(current_row && current_playing_list){
        ...
        GtkListBoxRow *n=gtk_list_box_get_row_at_index(current_playing_list, i+1); if(n) jouer_musique(n); else set_status("Fin liste.",0);
    }
}
void on_btn_prev_clicked(GtkButton *b, gpointer u) { (void)b; (void)u;
    if(current_row && current_playing_list){
        int i=gtk_list_box_row_get_index(current_row); if(i>0){ GtkListBoxRow *p=gtk_list_box_get_row_at_index(current_playing_list, i-1); if(p) jouer_musique(p); }
    }
}
...
static void activate(GtkApplication *app, gpointer u) { (void)u;
    ...
    gtk_window_set_title(GTK_WINDOW(main_window), "Projet Musique - V20 (Design)");
    ...
    // PL (Ajout classe bouton)
    GtkWidget *p2 = gtk_box_new(GTK_ORIENTATION_VERTICAL, 10); GtkWidget *b_pl = gtk_button_new_with_label("➕ Nouvelle Playlist");
    gtk_widget_add_css_class(b_pl, "text-button"); // POUR LE CSS
    ...
    GtkWidget *p3 = gtk_box_new(GTK_ORIENTATION_VERTICAL, 10); GtkWidget *s4 = gtk_scrolled_window_new(); gtk_widget_set_vexpand(s4, TRUE); list_box_artistes = gtk_list_box_new(); gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(s4), list_box_artistes); gtk_box_append(GTK_BOX(p3), s4); gtk_stack_add_titled(GTK_STACK(stack), p3, "art", "Artistes");
    ...
    gtk_frame_set_child(GTK_FRAME(frm), scrolled_window_paroles); gtk_box_append(GTK_BOX(p4), frm);
    gtk_stack_add_titled(GTK_STACK(stack), p4, "lec", "Lecture");

    // FOOTER (Classe CSS ajoutée)
...
int main(int c, char **v) { GtkApplication *a = gtk_application_new("com.projet.v20", G_APPLICATION_DEFAULT_FLAGS); g_signal_connect(a, "activate", G_CALLBACK(activate), NULL); int s = g_application_run(G_APPLICATION(a), c, v); ma_engine_uninit(&engine); g_object_unref(a); return s; }
