...
// --- VARIABLES GLOBALES (Définites ici pour de vrai) ---
...
static void activate(GtkApplication *app, gpointer u) { (void)u;
    ...
    gtk_window_set_title(GTK_WINDOW(main_window), "Projet Musique - V23 (Modulaire)");
    ...
    GtkWidget *stack = gtk_stack_new(); GtkWidget *sb = gtk_stack_sidebar_new(); gtk_stack_sidebar_set_stack(GTK_STACK_SIDEBAR(sb), GTK_STACK(stack));
    ...
    g_signal_connect(dd_tri, "notify::selected", G_CALLBACK(on_sort_changed), NULL); gtk_box_append(GTK_BOX(box_ctrl), dd_tri);
    GtkWidget *b_add = gtk_button_new_with_label("📂 Importer"); gtk_widget_add_css_class(b_add, "text-button");
    g_signal_connect(b_add, "clicked", G_CALLBACK(on_add_clicked), NULL); gtk_box_append(GTK_BOX(box_ctrl), b_add);
...
int main(int c, char **v) { GtkApplication *a = gtk_application_new("com.projet.v23", G_APPLICATION_DEFAULT_FLAGS); g_signal_connect(a, "activate", G_CALLBACK(activate), NULL); int s = g_application_run(G_APPLICATION(a), c, v); g_object_unref(a); return s; }
