...
// Définition des variables globales du module
...
void on_supprimer_clicked(GtkButton *b, gpointer u) {
    ...
    if(c){
        // Si c'est le son en cours, on arrête (simplification)
        stop_musique();
        supprimer_bdd(c);
    }
...
// V26 : Fonction mise à jour (Album + Durée)
void ajouter_ligne_visuelle_generique(GtkListBox *lb, const char *c, const char *t, const char *a, const char *alb, int dur, int pl) {
    ...
    char *txt = g_markup_printf_escaped(
        "<b>%s</b>\n<span size='small' foreground='gray'>%s • %s</span>",
        t, a, alb ? alb : "Inconnu");
    ...
    GtkWidget *l_dur = gtk_label_new(txt_dur);
    gtk_widget_set_margin_end(l_dur, 10);
    g_free(txt_dur);
...
void charger_catalogue() {
    ...
    while(sqlite3_step(s)==SQLITE_ROW) {
        ...
        ajouter_ligne_visuelle_generique(GTK_LIST_BOX(list_box_catalogue),
            ...
            (const char*)sqlite3_column_text(s,3), // Album
            sqlite3_column_int(s,4),               // Durée
            1);
    }
